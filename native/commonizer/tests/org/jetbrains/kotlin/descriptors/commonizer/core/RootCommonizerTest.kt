...
class RootCommonizerTest : AbstractCommonizerTest<CirRoot, CirRoot>() {
    ...
    fun allAreNative() = doTestSuccess(
        expected = KONAN_BUILT_INS.toMock(
            SharedCommonizerTarget(
                setOf(
                    LeafCommonizerTarget("ios_x64", KonanTarget.IOS_X64),
                    LeafCommonizerTarget("ios_arm64", KonanTarget.IOS_ARM64),
                    LeafCommonizerTarget("ios_arm32", KonanTarget.IOS_ARM32)
                )
            )
        ),
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_x64", KonanTarget.IOS_X64)),
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_arm64", KonanTarget.IOS_ARM64)),
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_arm32", KonanTarget.IOS_ARM32))
    )
    ...
    fun misconfiguration1() = doTestSuccess(
        ...
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_x64")),
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_arm64", KonanTarget.IOS_ARM64)),
        KONAN_BUILT_INS.toMock(LeafCommonizerTarget("ios_arm32", KonanTarget.IOS_ARM32))
    )
    ...
    fun misconfiguration2() = doTestSuccess(
        ...
        JVM_BUILT_INS.toMock(LeafCommonizerTarget("jvm1", KonanTarget.IOS_X64)),
        JVM_BUILT_INS.toMock(LeafCommonizerTarget("jvm2")),
        JVM_BUILT_INS.toMock(LeafCommonizerTarget("jvm3"))
    )
